      ...
      <div className="pt-20 px-4">
        <div className="flex flex-col items-center">
          <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-orange-500 shadow-lg">
            <img
              src={`http://localhost:5000/${user.profilePicture || 'uploads/default-profile.png'}`}
              alt={user.name}
              className="w-full h-full object-cover"
            />
          </div>
          
          <h1 className="mt-4 text-2xl font-bold text-[#463C33]">{user.name}</h1>
          ...
          <div className="mt-6 flex space-x-12">
            <div className="text-center">
              <span className="block text-2xl font-bold text-orange-500">{user.totalRecipes || 0}</span>
              <span className="text-sm text-gray-600">Recipes</span>
            </div>
            <div className="text-center">
              <span className="block text-2xl font-bold text-orange-500">{user.totalLikes || 0}</span>
        ...
        <div className="overflow-x-auto no-scrollbar">
          <div className="flex space-x-4 pb-4">
            {userRecipes.map((recipe) => (
              <div
                key={recipe._id}
                className="flex-shrink-0 w-64 bg-[#463C33] rounded-lg shadow-md overflow-hidden cursor-pointer"
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <button 
                    className="w-full bg-white text-[#463C33] text-xs font-bold rounded-full py-1.5 px-3 hover:bg-gray-100 transition-colors"
                    onClick={(e) => {
                      e.stopPropagation();
